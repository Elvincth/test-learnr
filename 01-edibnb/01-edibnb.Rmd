...
# print(knitr::knit_hooks$get("tutorial"))
...
```{r test}
ex("Hello, worl ebr gen tb eg4e vthr nteg thn g4tymn hbv erbntb ev btnybgve nmytnhbg rvfbny b4ntrhy jhdh v 4b35h \n Hello")
```

```{r ex1, exercise=TRUE, eval=FALSE}
What is 1+1
```
...
```{r glimpse-data, mark=15, exercise=TRUE}
glimpse(edibnb)
```
...
```{r price-histogram-solution, exercise.reveal_solution = FALSE}
...
You'll see in the above code that R gives us a couple of _warnings_. Let's analyse these:
